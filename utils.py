...
def get_train_wavs_lables():
    conf = Config()
    wav_files, text_labels = do_get_wavs_lables(conf.get("FILE_DATA").train_wav_path,
                                                conf.get("FILE_DATA").train_label_file)
...
def get_test_wavs_lables():
    conf = Config()
    wav_files, text_labels = do_get_wavs_lables(conf.get("FILE_DATA").test_wav_path,
                                                conf.get("FILE_DATA").test_label_file)
    print(wav_files[0], text_labels[0])
    # wav/train/A11/A11_0.WAV -> 绿 是 阳春 烟 景 大块 文章 的 底色 四月 的 林 峦 更是 绿 得 鲜活 秀媚 诗意 盎然
    print("wav:", len(wav_files), "label", len(text_labels))
    return wav_files, text_labels
